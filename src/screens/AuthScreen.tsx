...
export function AuthScreen({ lang, setLang }: Props) {
    ...
    try {
      if (mode === 'signup') {
        ...
        if (data.user && !data.session) {
          setError(
            lang === 'KH'
              ? 'Email Confirmation នៅតែបើក! សូមចូល Supabase Dashboard បិទ "Confirm Email"'
              : 'Email Confirmation is still ON! Turn it off in Supabase Dashboard'
          );
          setBusy(false);
          return;
        }
  ...
  return (
    <div
      style={{
        minHeight: '100dvh',
        background: `linear-gradient(160deg, ${COLORS.primary} 0%, ${COLORS.primaryDark} 50%, ${COLORS.neutral900} 100%)`,
        display: 'flex',
    ...
    >
      ...
      <div style={{ padding: '60px 24px 20px', textAlign: 'center', position: 'relative', zIndex: 1 }}>
        ...
        <h1 className={lang === 'KH' ? 'kh' : 'en'} style={{ fontSize: 28, fontWeight: 700, color: '#fff', marginBottom: 4 }}>
          {t('appName', lang)}
        </h1>
        <p className={lang === 'KH' ? 'kh' : 'en'} style={{ fontSize: 14, color: 'rgba(255,255,255,0.7)' }}>
          {mode === 'signin' ? t('welcome', lang) : t('signUp', lang)}
        </p>
      </div>
      ...
      >
        ...
        >
          {(['signin', 'signup'] as const).map((m) => (
            <button
              key={m}
              onClick={() => { setMode(m); setError(''); setErrorDetail(''); }}
              className="btn-press"
              style={{
                flex: 1,
                padding: '10px',
                ...
                color: mode === m ? '#fff' : COLORS.neutral500,
                fontSize: 14,
                fontWeight: 600,
                cursor: 'pointer',
        ...
        <div style={{ marginBottom: 14, position: 'relative' }}>
          ...
          <button
            ...
            style={{ position: 'absolute', right: 12, top: 10, border: 'none', background: 'none', cursor: 'pointer', padding: 4 }}
          >
