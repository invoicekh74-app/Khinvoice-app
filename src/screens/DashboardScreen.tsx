...
import { TrendingUp, TrendingDown, Wallet, FileText, ArrowRight, LogOut, Bell } from 'lucide-react';
    ...
    <div>
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'relative', zIndex: 1, marginBottom: 20 }}>
          <div>
            <p className={lang === 'KH' ? 'kh' : 'en'} style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)', margin: 0 }}>
              {t('welcome', lang)}
            </p>
            <h1 className={lang === 'KH' ? 'kh' : 'en'} style={{ fontSize: 20, fontWeight: 700, color: '#fff', margin: 0, marginTop: 2 }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            <p className={lang === 'KH' ? 'kh' : 'en'} style={{ fontSize: 12, color: 'rgba(255,255,255,0.8)', margin: 0 }}>
              {t('netProfit', lang)} · {t('thisMonth', lang)}
            </p>
          </div>
          <p className="en" style={{ fontSize: 32, fontWeight: 700, color: '#fff', margin: 0, letterSpacing: -0.5 }}>
            {fmt(netProfit)}
          </p>
          <div style={{ display: 'flex', gap: 16, marginTop: 12 }}>
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <p className={lang === 'KH' ? 'kh' : 'en'} style={{ fontSize: 11, color: 'rgba(255,255,255,0.7)', margin: 0 }}>
                  {t('income', lang)}
                </p>
              </div>
              <p className="en" style={{ fontSize: 16, fontWeight: 600, color: '#fff', margin: 0, marginTop: 2 }}>
                {fmt(totalIncome)}
              </p>
            </div>
      ...
      <div style={{ padding: '20px 16px 20px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 24 }}>
          {([
            ...
            >
              ...
              <span className={lang === 'KH' ? 'kh' : 'en'} style={{ fontSize: 10, fontWeight: 600, color: a.color, textAlign: 'center', lineHeight: 1.3 }}>
                {a.label}
              </span>
            </button>
          ))}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <h2 className={lang === 'KH' ? 'kh' : 'en'} style={{ fontSize: 16, fontWeight: 700, color: COLORS.neutral800, margin: 0 }}>
            {t('recentTransactions', lang)}
          </h2>
          <button
            onClick={() => onNavigate('transactions')}
            style={{ border: 'none', background: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}
          >
            <span className={lang === 'KH' ? 'kh' : 'en'} style={{ fontSize: 12, color: COLORS.primary, fontWeight: 600 }}>
              {t('all', lang)}
            </span>
        ...
        {recentTx.length === 0 ? (
          ...
          >
            <p className={lang === 'KH' ? 'kh' : 'en'} style={{ fontSize: 14, color: COLORS.neutral400 }}>
                ...
                >
                  <div
                    style={{
                      ...
                      height: 40,
                      borderRadius: 12,
                      background: isIncome ? COLORS.successTint : COLORS.dangerTint,
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                      flexShrink: 0,
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p className={lang === 'KH' ? 'kh' : 'en'} style={{ fontSize: 14, fontWeight: 600, color: COLORS.neutral800, margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {tx.description}
                    </p>
                    <p className="en" style={{ fontSize: 11, color: COLORS.neutral400, margin: 0, marginTop: 2 }}>
                      {tx.transaction_date}
                    </p>
                  </div>
